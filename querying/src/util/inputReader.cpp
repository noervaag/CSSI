 ...
 */
	...
	while (std::getline(insemfile, line))
	{
	    ...
	    {
		if ((num_elements + 1) > EMBED_DIM_HD) {
		    std::cerr << "Line higher dim than EMBED_DIM_HD read" << std::endl;
		    return 1;
		}
		corpus.docvec.at(num_sem).second[num_elements] = std::stod(elem);
		num_elements++;
	    }
		...
		std::cerr << "Line with different dim than EMBED_DIM_HD read" << std::endl;
		return 1;
	    }
	    num_sem++;
	}
	...
	EmbeddingHD min_elem;
	min_elem.fill(DBL_MAX);
	EmbeddingHD max_elem;
	max_elem.fill(-DBL_MAX);
    ...
    // Set initial capacity to save resizing cost
    corpus.emb_ld.reserve(Constants::DEFAULTCORPSIZE);
	...
	while (std::getline(inldsemfile, line))
	{
	    ...
	    while (std::getline(ss, elem, ' '))
	    {
		if ((num_elements+1) > EMBED_DIM_LD) {
		    std::cerr << "Line with higher dim than EMBED_DIM_LD read" << std::endl;
		    return 1;
		}
		emb[num_elements] = std::stod(elem); 
		num_elements++;
		...
		std::cerr << "Line with different ld_dim than EMBED_DIM_LD read" << std::endl;
		return 1;
	    }
	...
	EmbeddingLD min_elem;
	min_elem.fill(DBL_MAX);
	EmbeddingLD max_elem;
	max_elem.fill(-DBL_MAX);
	for(EmbeddingLD const &e : corpus.emb_ld ) {
...
int InputReader::readIdxFiles( std::string &idxdir, Corpus &corpus ) {
    ...
    std::vector < EmbeddingHD > tcentroids;
        ...
        while (std::getline(tcentroidfile, line))
	{
	    ...
	    {
		if ((num_elements+1) > EMBED_DIM_HD) {
		    std::cerr << "Line with higher dim than EMBED_DIM_HD read" << std::endl;
		    return 1;
		}
		emb[num_elements] = std::stod(elem); 
		num_elements++;
    ...
    std::vector < EmbeddingLD > tredcentroids;
