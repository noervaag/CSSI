...
namespace
{
    ...
    {
        uint32_t k;
        std::vector< sstss::DocId > query_points;
    ...
    long double avg(std::vector< long double > const& scores)
    {
        double s = std::accumulate (scores.cbegin(), scores.cend(), 0.0,
				    []( long double i, long double p ){ return i + p; });
...
int main( int argc, char** argv ) {
	...
	if (vm.count(ARG_IDXDIR))
		...
		parameters.query_points.push_back(std::stoul(p));
	    }
        }
	else if (vm.count(ARG_RNDQ))
	{
       ...
       if (vm.count(ARG_L))
        ...
        if (vm.count(ARG_ALGORITHM))
        {
		    ...
		    alg->preprocess(parameters.m);
                    auto const elapsed_preprocess = std::chrono::high_resolution_clock::now() - start_preprocess;

                    auto start_q = std::chrono::high_resolution_clock::now();
		    numdist = 0;
		    if (q >= corpus.num_docs) {
			std::cout << "Query id negative or higher than #docs in corpus!\n";
			exit(1);
		    }
		    else {
		    ...
		    batches["error"].push_back(stats.error);
                ...
                stats.query = 0;
                stats.query_index = 0;
                stats.microseconds_pre = avg(batches["preprocess"]);
                stats.microseconds_q = avg(batches["query"]);
                stats.microseconds_all = avg(batches["all"]);
                stats.prunes = avg(batches["prunes"]);
		stats.prunes2 = avg(batches["prunes2"]);
		stats.numdist = avg(batches["numdist"]);
	...
	if ( resultvecs.size() > num_queries) { // Make sure more than one algorithm has been run
		...
		for (size_t j=0; j < resultvecs.at(i).size(); j++) {
		    // Uncomment if you want to see results of verification
	    ...
	    std::cout << "Result set verification performed, results are correct!\n";  
	}
    }
    // Uncomment if you want to see actual results (for two algorithms, one query only)
