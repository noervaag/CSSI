...
namespace sstss
{
    ...
    {
	EmbeddingHD min_elem(pivot_m, DBL_MAX);
	EmbeddingHD max_elem(pivot_m, -DBL_MAX);
	for(Doc const &d : dv ) {
    ...
    */
	...
	for(size_t index = 0; index < pivot_m; ++index)
	    {
		newSemRect.m_min[index] = std::min(rectA.m_min[index], rectB.m_min[index]);
		newSemRect.m_max[index] = std::max(rectA.m_max[index], rectB.m_max[index]);
	    }
...
#define rMAX(a, b) ((a) > (b) ? (a) : (b))
    ...
    double S2RIndex::minSemDist(SemRect const& semRect, EmbeddingHD const& q) 
    {
	double minTD = -1;
